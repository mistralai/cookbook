...
def post_review(
    ...
    for lc in line_comments:
        ...
        if not isinstance(line, int) or line < 1 or (total_lines > 0 and line > total_lines):
            ...
            continue
        if added_lines_set is not None:
            # For a range comment, accept if any line in the range is in the diff.
            if isinstance(start_line, int) and start_line < line:
                range_in_diff = bool(set(range(start_line, line + 1)) & added_lines_set)
            else:
                range_in_diff = line in added_lines_set
            if not range_in_diff:
                sl_label = f"{start_line}–" if isinstance(start_line, int) and start_line < line else ""
                print(f"    Line {sl_label}{line} not in diff — will post as PR comment.")
                fallback_lcs.append(lc)
                continue
        bundled_item: dict = {
            "path": filepath,
        ...
        if isinstance(start_line, int) and 1 <= start_line < line:
            bundled_item["start_line"] = start_line
            bundled_item["start_side"] = "RIGHT"
    ...
    if not ok:
        ...
        for bc in bundled:
            if not post_line_comment(bc["path"], bc["line"], bc["body"], bc.get("start_line")):
