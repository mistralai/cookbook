...
def get_text_embedding(input_text: str):
    embeddings_batch_response = cli.embeddings.create(model="mistral-embed", input=input_text)
...
async def callback(contents: str, user: str, instance: pn.chat.ChatInterface):
    if type(contents) is str:
        messages_objects = [
            ...
            if w.user != "System" and type(w.object) is not pn.chat.message._FileInputMessage
        ]
        messages = [
            {"role": "user" if w.user == "User" else "assistant", "content": w.object} for w in messages_objects
        ]
        ...
        response = cli.chat.stream(model="open-mistral-7b", messages=messages, max_tokens=1024, temperature=0.7)
        message = ""
...
chat_interface = pn.chat.ChatInterface(
    widgets=[pn.widgets.TextInput(), pn.widgets.FileInput(accept=".pdf")], callback=callback, callback_user="Mistral"
)
