"""GLM OpenAI-compatible text extraction provider."""
...
from extract_bench.inference.providers.extract.codex_deepseek_extract import (
    _GLM_5_2_MODEL,
    _GLM_API_BASE_URL,
    _GLM_EXTRACT_PRICING_PER_M,
)
...
@register_provider("glm_extract")
class GLMExtractProvider(Provider):
    """Structured extraction through the GLM 5.2 OpenAI-compatible endpoint."""
    ...
    def __init__(self, provider_name: str, base_config: dict[str, Any] | None = None):
        ...
        self._api_key = self.base_config.get("glm_api_key") or self.base_config.get("api_key")
        if not self._api_key:
            self._api_key = os.getenv("GLM_API_KEY")
        if not self._api_key:
            raise ProviderConfigError("GLM API key is required for glm_extract.")

        self._model: str = self.base_config.get("model", self.DEFAULT_MODEL)
        self._base_url: str = self.base_config.get("base_url", _GLM_API_BASE_URL)
        ...
        if self._input_mode != "parsed_text":
            raise ProviderConfigError("glm_extract supports input_mode='parsed_text' only.")
    ...
    def _pricing_snapshot(self) -> dict[str, Any]:
        return {
            "pricing_basis": "glm_5_2_serverless",
