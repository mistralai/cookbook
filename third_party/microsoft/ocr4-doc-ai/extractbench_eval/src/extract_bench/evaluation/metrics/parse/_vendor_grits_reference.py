...
def factored_2dmss(true_cell_grid, pred_cell_grid, reward_function):
    ...
    for true_row_num, pred_row_num in zip(true_row_nums, pred_row_nums, strict=False):
        for true_column_num, pred_column_num in zip(true_column_nums, pred_column_nums, strict=False):
...
def iou(bbox1, bbox2):
    ...
    # Convert numpy arrays to lists for newer pymupdf compatibility
...
def output_to_dilatedbbox_grid(bboxes, labels, scores):
    ...
    *dilated*, which means they are larger than the actual ground truth boxes.
    ...
    an individual model.
    """
    rows = [{"bbox": bbox} for bbox, label in zip(bboxes, labels, strict=False) if label == 2]
    columns = [{"bbox": bbox} for bbox, label in zip(bboxes, labels, strict=False) if label == 1]
    spanning_cells = [
        {"bbox": bbox, "score": 1} for bbox, label in zip(bboxes, labels, strict=False) if label in [4, 5]
    ]
    ...
    for _row_num, row in enumerate(rows):
        column_grid = []
        for _column_num, column in enumerate(columns):
    ...
    for matches, spanning_cell in zip(matches_by_spanning_cell, spanning_cells, strict=False):
